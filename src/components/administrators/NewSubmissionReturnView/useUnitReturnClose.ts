...
export const useUnitReturnClose = (dispatch: Dispatch<Action>): Subject<NewUnitCloseEvent> => {
  ...
  useEffect(() => {
    ...
    unitReturnClose$.current.pipe(
      ...
      exhaustMap(({ administratorId, submissionReturnId, adminComment }) => {
        return newSubmissionReturnService.closeSubmissionReturn(administratorId, submissionReturnId, adminComment).pipe(
          tap({
            next: newUnitReturn => {
