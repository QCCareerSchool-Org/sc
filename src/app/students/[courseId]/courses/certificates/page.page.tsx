...
const CertificatePage = async ({ params }: PageProps) => {
  ...
  const certificate: Certificate = await fetchCertificate(courseId, studentId);
    ...
    <>
        ...
        <div className="position-relative z-1 container text-center text-white">
          ...
          <div className="row justify-content-center">
            <div className="col-12 col-sm-10 col-md-8 col-lg-5">
              <div className="bg-white p-2 rounded-4 shadow text-black fw-bold mb-4">Congratulations, {certificate.firstName} {certificate.lastName}!</div>
            </div>
          </div>
      ...
      <section>
        <div className="container text-center">
          <div className=" bg-white text-dark rounded-3 border border-1 p-4 mt-4 mb-4">CERTIFICATION ID: {certificate.signature}</div>
        </div>
      </section>
      <div className="text-center">
        <div
          className="mx-auto" style={{ maxWidth: '1200px', boxShadow: '0 4px 24px rgba(0, 0, 0, 0.12)' }}
        >
          <CertificateWrapper name={certificate.firstName + ' ' + certificate.lastName} schoolName={certificate.schoolName} courseName={certificate.courseName} registrarSignatureUrl={registrarSignature} directorSignatureUrl={directorSignature} date={certificate.graduationDate} />
        </div>
      </div>
      <section>
        <div>
          <div className="bg-white text-dark rounded-3 border border-1 p-4 mt-4 mb-4 row justify-content-center">
            ...
            <div className="col-12 col-md-4">
              ...
              <p className="mb-0">{certificate.graduationDate.toLocaleDateString()}</p>
            </div>
          </div>
        </div>
      </section>
