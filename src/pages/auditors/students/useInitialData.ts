...
export const useInitialData = (dispatch: Dispatch<Action>, auditorId: number): void => {
  ...
  useEffect(() => {
    ...
    studentService.getAllStudents(auditorId).pipe(
      takeUntil(destroy$),
    ).subscribe({
      next: students => dispatch({ type: 'LOAD_DATA_SUCCEEDED', payload: students }),
      error: err => {
  ...
  }, [ dispatch, auditorId, studentService, navigateToLogin ]);
};
